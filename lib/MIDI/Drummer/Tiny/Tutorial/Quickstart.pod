...
C<acoustic_bass> & C<electric_bass> (Generic: C<kick>)

Snare-like things:

C<acoustic_snare> & C<electric_snare> (Generic: C<snare>), C<side_stick>, C<clap>
...
=head2 Miscellaneous
  ...
  click, bell (i.e. the metronome sound), tambourine, cowbell, vibraslap, etc.
...
etc. up to double-dotted 128th notes.
...
Either play the score with the built-in L<timidity++|https://timidity.sourceforge.net/|> functionality, plus a defined C<soundfont>:
...
And then play the score with timidity (where the C<cfg> file specifies the soundfont to be used):
